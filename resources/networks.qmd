...
Network data is different from other types of data we have encountered because it is made up of two related structure: an **edge list** and a **node list**, though the latter can be created from the former if the nodes do not have attributes. Within R these two data structures are represented by data frames:
...
This is the basic data structure of all network data. There may be hundreds of thousands of nodes or edges and dozens of attributes, but at its foundation, network data is node pairs and a set of unique nodes.
...
[^1]: By using `set.seed()` the layout algorithm will be done in the same way each time it is run.