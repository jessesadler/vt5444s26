...
2. Make changes to the documents in the folder, making sure to save the files as you go along using File -> Save, the save file button in RStudio, or the `Cmd/Ctrl + S` keyboard shortcut.[^1]
3. Add the changed files that you want to be a part of a commit to the *stage*.
4. *Commit* those changed files as a save state with a commit message.
5. Repeat steps 2–4.
6. Push your changes to GitHub repository.

::: {.callout-note}
## Quarto websites and Git
The workflow for developing Quarto websites follows this basic structure, but it involves a couple of more pieces: 

1. You will want to **building and previewing** the website into the workflow of making changes to your files (step 2).
2. Setting up the connection to GitHub is more complex because you want to not only have your files on GitHub but also build and serve a website.

For the particulars on working with Quarto websites and Git, see the [Quarto websites resources page](quarto-websites.qmd).
:::

There are many different ways you can go about doing these foundational steps. Adding in GitHub to the mix brings in further complexity. Do you create the repository on GitHub first or on your computer. Let's go through creating a repository on [GitHub first](#github-first), which makes things a bit easier because each step actually combines multiple steps into one. Then, we will go through how to get our [`r-notes` project on GitHub](#local-first), which is a bit more manual.
...
1. Go to GitHub and create a new repository
    ...
    2. Name the repository the **exact same name as the RStudio project folder**, such as `r-notes`.
...
1. **Make changes** to your project, saving the files as you go along.
2. **Check the output** of the changes.
    - [R scripts](rstudio-setup.qmd#scripts-and-reproducibility): check that the code is doing what you want. To be extra sure, restart your R session to make sure the code is not dependent on an object you created without writing it in the script.
    - [Quarto documents](quarto-docs.qmd): check that the quarto document Renders and looks like what you want.[^2]
    - [Quarto websites](quarto-websites.qmd#render-and-preview-your-website): check that the website builds correctly by rendering and previewing the site from the Build tab in the Environment panel.
3. Make a **commit** consisting of a defined set of changes. This is a two-step process.
    1. Add the files to the **stage** that you want to be part of a commit.
    2. Commit the staged files with a message describing the changes.
4. **Repeat steps 1–3** until you feel you have made good progress.
...
[^1]: This is usually the only and, therefore, the last step in working with files, but Git is more involved, as you will see on this page.

[^2]: Rendering a Quarto document is the same as restarting R and running an R script from top to bottom. If there are any problems, the render will not work. Doing this often makes it easier to identify where problems are occurring.

[^3]: If something does break, or you later find a problem, there is no need to panic. You can always make a change to fix things. However, changes, especially rewinding changes, are always easier when they are only on your local computer. When changes are on both GitHub and your computer, things get a little more complex.