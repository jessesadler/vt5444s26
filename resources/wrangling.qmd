...
```{r}
penguins |> 
  select(species, bill_len, bill_dep, sex) |> # <1>
  filter(!is.na(bill_len) | !is.na(bill_dep)) |> # <2>
  mutate(bill_ratio = bill_dep / bill_len) |> # <3>
  group_by(species, sex) |> # <4>
  summarize(avg_ratio = mean(bill_ratio), # <5>
            n = n(), # <6>
            .groups = "drop") |> # <7>
  arrange(desc(avg_ratio)) # <8>
```

1. Subset columns; not strictly necessary because of summarization further in pipeline.
2. Remove missing values for bill measurements.
3. Create new column for ratio of bill depth to bill length.
4. Group the data by species and sex.
5. Find the average bill ratio broken down by species and sex.
6. Count the number of observations in each group.
7. Drop the remaining grouping of sex.
8. Arrange the rows by the average ratio in descending order.
