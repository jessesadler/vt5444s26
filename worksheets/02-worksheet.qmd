...
Now you can see that `tibble`s by defualt only prints out the first 10 rows of the data frame and as many columns as fits in the console.
...
With a tibble that we read in with `read_csv()` we get two sets of overlapping information. We first get the data frame turned on its side (columns now appear as rows), which highlights the column names and variable types. Secondly, it shows how the columns were parsed by `read_csv()`.
...
Check out the [documentation for `slice_smaple()`](https://dplyr.tidyverse.org/reference/slice.html) to see other functions you might use.
